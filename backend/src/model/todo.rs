...
#[derive(sqlb::Fields, Clone, Default, Debug)]
pub struct TodoPatch {
    pub title: Option<String>,
    pub status: Option<TodoStatus>,
}

#[derive(sqlx::Type, PartialEq, Eq, Clone, Debug, Serialize)]
...
impl TodoMac {
    pub async fn create(db: &Db, utx: &UserCtx, data: TodoPatch) -> Result<Todo, model::Error> {
        ...
        //     .bind(123 as i64) // should come from user context
    ...
    pub async fn list(db: &Db, _utx: &UserCtx) -> Result<Vec<Todo>, model::Error> {
        // let sql = "selectsbid,cid,title, status from todo order by id desc";
        let sb = sqlb::select()
            .table(Self::TABLE)
            .columns(Self::COLUMNS)
            .order_by("!id");
